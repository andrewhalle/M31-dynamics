...
def get_j(sim, particle):
    com = sim.calculate_com()
    r = np.array([particle.x - com.x, particle.y - com.y, particle.z - com.z])
    v = np.array([particle.vx - com.vx, particle.vy - com.vy, particle.vz - com.vz])
    p = particle.m * v
    return np.cross(r, p)
...
def heartbeat_function(sim):
    ...
    if n_collisions > 0:
        for p in ps:
            ...
            if (get_j(sim, p)[2] < 0):
                flipped.append([sim.t, n_collisions, p.d, p.x, p.y, p.z, p.vx, p.vy, p.vz])
        for p in ps:
            sim.remove(id=p.id)
...
while i < maxtime:
    ...
    #######################
    if (i % timestep == 0):
        aligned.save("../logs/flipper/" + str(i).zfill(9) + ".log")
        print(str(int(np.round(i/maxtime*100))) + '%, t=' + str(i))
    i += 10

pickle.dump(collisions, open("../logs/flipper/collisions.txt", "wb"))
pickle.dump(flipped, open("../logs/flipper/flipped.txt", "wb"))
