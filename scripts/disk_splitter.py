...
i = 0
dmz_distances = pickle.load(open("../images/find_DMZ/data.txt", "rb"))
...
while i < 15000000:
    ...
    for p in ps:
        o = p.calculate_orbit()
        data.append([o.a, precess(sim, p)])
    data_pos = [a for a in data if a[1] >= 0]
    data_neg = [a for a in data if a[1] < 0]
    a_pos = [a[0] for a in data_pos]
    a_neg = [a[0] for a in data_neg]
    pr_pos = [pr[1] for pr in data_pos]
    pr_neg = [pr[1] for pr in data_neg]
    dmz = [a[1] for a in dmz_distances if a[0] == i][0]
    plt.scatter(a_pos, pr_pos, c='r', label="Prograde")
    plt.scatter(a_neg, pr_neg, c='b', label="Retrograde")
    #plt.axvline(x=dmz, c="black", ls="dashed", lw="2")
