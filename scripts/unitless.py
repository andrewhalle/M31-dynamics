...
initial_e = 0.50  #originally random() for suite of simulations
...
aligned.add(id=0, m=1)
aligned.softening = soften
i = 1
while i <= n_particles:
    #ann-marie code
    ...
    aligned.add(id=i, m=particle_mass, a=get_a(), e=initial_e, inc = inclination, omega=omega_1, Omega = Omega_1, M=np.random.uniform(0, 2*np.pi))
    i += 1
...
while i < maxtime:
    ...
    if (i % timestep < 0.001 or i % timestep > timestep - 0.001):
        aligned.save("logs/suite/" + sim_number + "/" + str(int(i)).zfill(9) + ".log")
        print(str(int(np.round(i/maxtime*100))) + '%, t=' + str(i))
    i += 0.1
    print(i)

pickle.dump(collision_events, open("logs/suite/" + sim_number + "/collision_events.txt", "wb"))
